...
export default class AddCourse extends React.Component {
    ...
    checkvalues = (event) => {
        ...
        const normalprice = parseInt(this.state.price.normal);
        const earlybirdprice = parseInt(this.state.price.early_bird);


        if (this.state.instructors.length === 0) {
            this.message = "Choose at least 1 instructor";
        }
        else if ((enddate - startdate) <= 0) {
            this.message = "Start date should be earlier than end date";
        }
        else if ((normalprice - earlybirdprice) <= 0) {
            this.message = "Normal price should be greater than early-bird price";
        }
        else {
            this.isValid = true;
        }
            ...
            <div className="myform">
                ...
                <Form onSubmit={this.submit} >
                    ...
                    <Form.Group >
                        ...
                        <Form.Control required as="textarea" rows={3} name="description" value={this.state.description} onChange={this.myChangeHandler} />
                    </Form.Group>

                    <h1>Dates</h1>
                    <Form.Group >
                        <Form.Label>Start date:</Form.Label>
                        <Form.Control required type="date" name="start_date" value={this.state.dates.start_date} onChange={this.handleDates} />
                    </Form.Group>
                    <Form.Group >
                        <Form.Label>End date:</Form.Label>
                        <Form.Control required type="date" name="end_date" value={this.state.dates.end_date} onChange={this.handleDates} />
                    </Form.Group>
                    <h1>Price</h1>
                    <Form.Group >
